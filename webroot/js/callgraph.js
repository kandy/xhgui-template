Xhgui.callgraph = function(container, data, options) {
    ...
    var nodeStyle = function(node) {
        ...
        return 'fill: ' + colors(ratio) + '; stroke: none'
    };
    var nodeLabelStyle = function(node) {
        var ratio = node.value / data.total * 100;
        return 'font-size: ' + textSize(ratio) + 'em; font-color: #FFF;';
    }
    ...
    for (var i = 0, len = data.nodes.length; i < len; i++) {
        ...
        let label = node.name.replace('\\{closure}', '').split('\\').at(-1);
        g.addNode(node.name, {
            label: label + '\n' + Xhgui.formatNumber(node.value),
